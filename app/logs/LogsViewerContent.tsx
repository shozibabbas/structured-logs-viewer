...
export function LogsViewerContent() {
  ...
  const initialPacketFilter = useMemo(() => {
    const packetParam = searchParams.get('packet');
    return packetParam || 'ALL';
  }, [searchParams]);

  const [filterLevel, setFilterLevel] = useState<string>('ALL');
  const [filterFile, setFilterFile] = useState<string>('ALL');
  const [filterPacket, setFilterPacket] = useState<string>(initialPacketFilter);
  const [filterExtractMode, setFilterExtractMode] = useState<string>('ALL');
  const [searchQuery, setSearchQuery] = useState<string>('');

  // Extract unique levels and modes
  ...
  const filteredLogs = useMemo(
    ...
    [logs, filterLevel, filterFile, filterPacket, filterExtractMode, searchQuery]
  );
